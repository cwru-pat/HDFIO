...
public:
  HDFIO(hsize_t *mem_dims_in, IT rank_in)
  {
    initialize(mem_dims_in, rank_in, HDFIO_VERBOSE_OFF);
  }

  HDFIO(hsize_t *mem_dims_in, IT rank_in, int verbosity_level_in)
  {
    initialize(mem_dims_in, rank_in, verbosity_level_in);
  }

  void initialize(hsize_t *mem_dims_in, IT rank_in, int verbosity_level_in)
  {
    ...
    HDFIO_DEBUG_COUT << "sizeof(hsize_t) = " << sizeof(hsize_t) << std::endl << std::flush;
    HDFIO_DEBUG_COUT << "sizeof(mem_dims_in) = " << sizeof(mem_dims_in) << std::endl << std::flush;

    HDFIO_DEBUG_COUT << "Allocating internal variables..." << std::endl << std::flush;
    ...
    HDFIO_DEBUG_COUT << "Initializing mem_dims..." << std::endl << std::flush;
    for(IT i=0; i<rank; ++i)
    {
    ...
    HDFIO_DEBUG_COUT << "Creating mem_dspace_id..." << std::endl << std::flush;
    mem_dspace_id = H5Screate_simple(rank, mem_dims, NULL);

    setHyperslabParameters(2);
  }

  ~HDFIO()
  {
    HDFIO_DEBUG_COUT << "Closing HDFIO..." << std::endl << std::flush;
  ...
  void setOutputType(hid_t output_type_in)
  {
  ...
  {
    HDFIO_DEBUG_COUT << "Setting stride..." << std::endl << std::flush;
    for(IT i=0; i<rank; ++i)
      stride[i] = stride_in;
  ...
  void setHyperslabParameters(hsize_t *stride_in)
  {
    HDFIO_DEBUG_COUT << "Setting additional hyperslab parameters..." << std::endl << std::flush;
    for(IT i=0; i<rank; ++i)
    {
  ...
  HDFIOFileInfo getFileInfo(std::string file_name, std::string dataset_name)
  {
    ...
    file_info.dataset_id = H5Dcreate(file_info.file_id, dataset_name.c_str(), output_type, file_info.file_dspace_id, H5P_DEFAULT, file_info.data_compression_plist, H5P_DEFAULT);
    HDFIO_DEBUG_COUT << "Done!" << std::endl << std::flush;
  ...
  void writeArrayToFile(RT array[], std::string file_name, std::string dataset_name)
  {
    ...
    status = H5Dwrite(file_info.dataset_id, output_type, mem_dspace_id, file_info.file_dspace_id, H5P_DEFAULT, array);
    HDFIO_DEBUG_COUT << "Done!" << std::endl << std::flush;

    /* Reset the selection for the memory dataspace fid */
    //status = H5Sselect_none(mem_dspace_id);
